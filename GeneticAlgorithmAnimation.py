...
N = 12;  # Matris Boyutu for 11*11 matrix
K = 30;  # Popülasyon Sayısı
M = int((N * N) / 3 + 1)    # Yol Sayısı
...
def path_to_indices(path,food,location,text):       # Yazdırma Animasyonu
    ...
    this_location = list(location)
    #arrow_dictionary[tuple(location)] = " s "
    this_food = list(food)
    for i in path[0]:
        #        print(i)
        if i == 1:
            ...
            #arrow_dictionary[tuple(this_location)] = ' ◄ '
...
def genetic_algorithm(population, food):        # işi yapan algoritma
    ...
    bestindividuals = []
    time.sleep(0.5)
    print("animation will be played in 5 seconds. animation works only in cmd!")
    time.sleep(5)
    path_to_indices(population[0], food, location, 'Random Generated First Individual')
    ...
    bestindividuals.append(found)
    time.sleep(5)

    try:
        middle = bestindividuals[int(len(bestindividuals) / 2)]
        path_to_indices(middle, food, location, 'Example of Some Middle Generations individual :')
        path_to_indices(found, food, location, 'After '+str(k)+ ' generations best individual is :')
    except:
        pass
